...
Version: 4.11.37
*/
define("ELISQLREPORTS_VERSION", '4.11.37');
...
$GLOBALS["ELISQLREPORTS"] = array("query_times" => array(), "reports_keys" => array(), "encode" => '/[\?\-a-z\: \.\=\/\&\_]/ie', "backup_file" => false, "plugin_home" => "http://wordpress.ieonly.com/", "updated_images_path" => "wp-content/plugins/update/images/", "Report_SQL" => "",
...
"Plugin Updates"=>'<div id="findUpdates"><center>Searching for updates ...<br /><img src="'.$GLOBALS["ELISQLREPORTS"]["images_path"].'wait.gif" alt="Wait..." /><br /><input type="button" value="Cancel" onclick="document.getElementById(\'findUpdates\').innerHTML = \'Could not find server!\';" /></center></div><script type="text/javascript" src="'.str_replace("http:", "", $GLOBALS["ELISQLREPORTS"]["plugin_home"]).$GLOBALS["ELISQLREPORTS"]["updated_images_path"].'?js='.ELISQLREPORTS_VERSION.'&p=ELISQLREPORTS"></script>',
"Plugin Links"=>'<a target="_blank" href="https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=7K3TSGPAENSGS"><img id="pp_button" src="'.$GLOBALS["ELISQLREPORTS"]["images_path"].'btn_donateCC_WIDE.gif" border="0" alt="Make a Donation with PayPal"></a>
<ul class="sidebar-links">
	...
	<li><a target="_blank" href="'.$GLOBALS["ELISQLREPORTS"]["plugin_home"].'category/my-plugins/sql-reports/">Plugin Posts on Eli\'s Blog</a></li>
</ul>',
...
/*!40111 SET @OLD_SQL_NOTES=@@SQL_NOTES, SQL_NOTES=0 */;');
			...
			if (strlen($GLOBALS["ELISQLREPORTS"]["settings_array"]["backup_email"])) {
				...
				if (mail($GLOBALS["ELISQLREPORTS"]["settings_array"]["backup_email"], $return, $message.$content."\r\n\r\n--$uid--", $headers))
					...
					mail($GLOBALS["ELISQLREPORTS"]["settings_array"]["backup_email"], $return, $message.strlen($content)." bytes is too large to attach but you can download it <a href='".admin_url("admin.php?page=ELISQLREPORTS-settings&Download_SQL_Backup=".basename($backup_file))."'>here</a>.\r\n\r\n--$uid--", $headers);
			}
		} else
...
if (!function_exists('ur1encode')) { function ur1encode($url) {
	return preg_replace($GLOBALS["ELISQLREPORTS"]["encode"], '\'%\'.substr(\'00\'.strtoupper(dechex(ord(\'\0\'))),-2);', $url);
}}
...
function setButtonValue(newval) {
	...
	elseif (isset($GLOBALS["ELISQLREPORTS"]["query_times"][$SQLkey]["time"]) && isset($GLOBALS["ELISQLREPORTS"]["query_times"][$SQLkey]["rows"]) && $GLOBALS["ELISQLREPORTS"]["query_times"][$SQLkey]["rows"])
		echo 'Query returned '.$GLOBALS["ELISQLREPORTS"]["query_times"][$SQLkey]["rows"].' rows in '.substr($GLOBALS["ELISQLREPORTS"]["query_times"][$SQLkey]["time"], 0, 6).' seconds.<br /><br />';
	return $Report_SQL;
}
...
function ELISQLREPORTS_settings() {
	...
	<div class="inside" style="margin: 10px;"><div style="float: left; margin: 5px;">Default Styles for Report DIV:<br /><textarea name="ELISQLREPORTS_default_styles"cols=30 rows=2>'.$GLOBALS["ELISQLREPORTS"]["settings_array"]["default_styles"].'</textarea></div><div style="float: left; margin: 5px;">Place <b>SQL Reports</b> Menu Item:<br />';
	foreach (array("above <b>Appearance</b>", "below <b>Settings</b>") as $mg => $menu_group)
		echo '<div style="padding: 4px 24px;" id="menu_group_div_'.$mg.'"><input type="radio" name="ELISQLREPORTS_menu_group" value="'.$mg.'"'.($GLOBALS["ELISQLREPORTS"]["settings_array"]["menu_group"]==$mg||$mg==0?' checked':'').' />'.$menu_group.'</div>';
...
//	else echo md5(serialize($args)).'='.serialize($args);
	...
	echo "</select><input type=submit value=Run /></div><script>function make_restore() {document.getElementById('makebackup').innerHTML='$js';}</script><br />$files\n</form></div></div></div></div></div>";
}
...
function ELISQLREPORTS_menu() {
	...
	if (current_user_can("activate_plugins")) {
		...
		if (!(isset($GLOBALS["ELISQLREPORTS"]["settings_array"]["menu_group"]) && is_numeric($GLOBALS["ELISQLREPORTS"]["settings_array"]["menu_group"])))
			$GLOBALS["ELISQLREPORTS"]["settings_array"]["menu_group"] = 0;
		...
		if (isset($_POST["ELISQLREPORTS_backup_dir"]) && strlen(trim($_POST["ELISQLREPORTS_backup_dir"])) && is_dir($_POST["backup_dir"]) && ($_POST["ELISQLREPORTS_backup_dir"] != $GLOBALS["ELISQLREPORTS"]["settings_array"]["backup_dir"]))
		...
		if (isset($_POST["ELISQLREPORTS_menu_group"]) && is_numeric($_POST["ELISQLREPORTS_menu_group"]) && isset($_POST["ELISQLREPORTS_menu_sort"]) && is_numeric($_POST["ELISQLREPORTS_menu_sort"])) {
			$GLOBALS["ELISQLREPORTS"]["settings_array"]["menu_group"] = intval($_POST["ELISQLREPORTS_menu_group"]);
		...
		if (!function_exists("add_object_page") || $GLOBALS["ELISQLREPORTS"]["settings_array"]["menu_group"] == 1)
...
function ELISQLREPORTS_set_plugin_action_links($links_array, $plugin_file) {
	if (strlen($plugin_file) > 10 && $plugin_file == substr(__file__, (-1 * strlen($plugin_file))))
		$links_array = array_merge(array(ELISQLREPORTS_link("Create SQL Report", "&", "welcome-add-page")), $links_array);
	return $links_array;
}
...
function ELISQLREPORTS_set_plugin_row_meta($links_array, $plugin_file) {
	if (strlen($plugin_file) > 10 && $plugin_file == substr(__file__, (-1 * strlen($plugin_file))))
		$links_array = array_merge($links_array, array(ELISQLREPORTS_link("Donate", "https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=8VWNB5QEJ55TJ", "heart")));
	return $links_array;
}
